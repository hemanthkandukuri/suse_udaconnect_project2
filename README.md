Please refer [README2.md](README2.md) for directory structure, improvements, DockerHub and other details.
# UdaConnect
## Overview
### Background
...
### Steps
Run the below command which will deploy the entire application, by running all the deployment files and 
seeding the Postgres DB as well.
`sh deployment/deploy.sh`

### Verifying it Works
Once the project is up and running, you should be able to see 3 deployments and 3 services in Kubernetes:
`kubectl get pods` - should return the list of pods running in the cluster
`kubectl get services` -  should return the list services configured, forwarded and exposed in the cluster.
For both of them refer the images in `docs\pods_screenshot.png` & `docs\services_screenshot.png`
